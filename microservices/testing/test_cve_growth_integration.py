...
# TESTS
def test_get_growth_returns_200():
    url = f"{BASE_URL}/v1/growth/{COMPANY_NAME_1}"
    params = {"days": 7}
...
def test_get_growth_invalid_company_404():
    url = f"{BASE_URL}/v1/growth/UnknownCorp"
    response = requests.get(url, timeout=15)
    
    assert response.status_code == 404
    assert "error" in response.json()

def test_post_to_growth_route_returns_error():
    url = f"{BASE_URL}/v1/growth/{COMPANY_NAME_1}"
