data "archive_file" "delete_watchlist_zip" {
  type        = "zip"
  output_path = "${path.module}/deleteSubscriptionWatchlist.zip"

  source {
    content  = file("${path.module}/../src/deleteSubscriptionWatchlist.py")
    filename = "deleteSubscriptionWatchlist.py"
  }
...
resource "aws_lambda_function" "delete_watchlist" {
  function_name    = "subscription-delete-watchlist"
  role             = aws_iam_role.subscription_lambda_role.arn
  handler          = "deleteSubscriptionWatchlist.lambda_handler"
  runtime          = "python3.12"
  filename         = data.archive_file.delete_watchlist_zip.output_path
  source_code_hash = data.archive_file.delete_watchlist_zip.output_base64sha256
  timeout          = 300
  layers = [
...
resource "aws_apigatewayv2_integration" "delete_watchlist_int" {
  api_id           = var.api_id
  integration_type = "AWS_PROXY"
  integration_uri  = aws_lambda_function.delete_watchlist.invoke_arn
}

resource "aws_apigatewayv2_route" "delete_watchlist_route" {
  api_id    = var.api_id
  route_key = "DELETE /v2/watchlist/{id}"
  target    = "integrations/${aws_apigatewayv2_integration.delete_watchlist_int.id}"
}

resource "aws_lambda_permission" "delete_watchlist_perm" {
