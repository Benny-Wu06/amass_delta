data "archive_file" "company_summary_zip" {
  type        = "zip"
  output_path = "${path.module}/company_summary.zip"

  source {
    content  = file("${path.module}/../src/company_summary.py")
    filename = "company_summary.py"
  }
...
resource "aws_lambda_function" "company_summary" {
  ...
  runtime          = "python3.12"
  filename         = data.archive_file.company_summary_zip.output_path
  source_code_hash = data.archive_file.company_summary_zip.output_base64sha256
  layers = [
